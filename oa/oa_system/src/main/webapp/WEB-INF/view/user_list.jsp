...
<body>
	<nav class="breadcrumb">
		...
		管理员管理 <span class="c-gray en">&gt;</span> 管理员列表 <a
			...
			href="javascript:location.replace(location.href);" id="refresh"
	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加管理员</a></span> <span class="r">共有数据：<strong id="total"></strong> 条
			</span>
		</div>
		...
		<div style="margin-top: 15px" id="userPage"></div>
	</div>
	<!--_footer 作为公共模版分离出去-->
	...
	<!--请在下方写此页面业务相关的脚本-->
	<script type="text/javascript"
		src="${ctx}/lib/My97DatePicker/4.8/WdatePicker.js"></script>
	<script type="text/javascript"
...
function showData(users){
	...
	for(var i = 0;i<users.length;i++){
		...
		html +="<td><input id='checkboxs' type='checkbox' value='"+u.id+"' name='check'></td>";
		html +="<td>"+u.id+"</td>";
		...
		html +="<td class='td-manage'><a title='编辑' href='javascript:;'onclick='admin_edit("+u.id+")'class='ml-5'";
		html +="style='text-decoration: none'><i class='Hui-iconfont'>&#xe6df;</i></a> ";
		html +="<a title='删除' href='javascript:;' onclick='admin_del(this,"+u.id+")' class='ml-5'style='text-decoration: none'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
		html +="</td>";
		html +="</tr>";
	}
	//添加表格中
	        ...
	        laypage({
	          ...
	          jump: function(obj, first){ //触发分页后的回调
	            ...
	            if(!first){ //点击跳页触发函数自身，并传递当前页：obj.curr
	            	menulists("${ctx}/user/list.do",obj.curr);
	            }
	          }
			})
		}
...
/*管理员-增加*/
function admin_add(){
	layer_show("添加用户","${ctx}/user/add.do","800","500");
}
/*管理员-删除*/
...
function admin_edit(uid){
	layer_show("修改用户","${ctx}/user/edit.do?id="+uid,"800","500");
}
