...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			...
			<input type="text" class="input-text"  value="${user.usercode}" placeholder="" id="adminName" <c:if test="${user.id!=null}">disabled</c:if>  name="usercode">
		</div>
	</div>
	<div class="row cl">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			...
			<input	 type="<c:if test="${user.id!=null}">password</c:if><c:if test="${user.id==null}">text</c:if>"  class="input-text" autocomplete="off" value="${user.password}"  placeholder="确认新密码" id="password2" name="password">
		</div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="roleId" size="1">
				<c:forEach items="${roles}"  var="role">
					<option  ${user.roleId eq role.roleId ? 'selected':''} value="${role.roleId}">${role.name}</option>
				</c:forEach>
			</select>
			</span> </div>
	</div>
	<div class="row cl">
		<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
			<input class="btn btn-primary radius" onclick="saveOrUpdateUser()" type="button" value="&nbsp;&nbsp;提交&nbsp;&nbsp;">
		</div>
	</div>
	</form>
</article>
...
function saveOrUpdateUser(){
	...
	//使用ajax进行用户数据更新
	$.post("${ctx}/user/update.do",formData,function(result){
		if(result.code==1){
			//弹框
